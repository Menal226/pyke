...
async def test_by_puuid(pyke_client: Pyke, respx_mock: MockRouter):
    respx_mock.get(
        f"{BASE}/riot/account/v1/accounts/by-puuid/{PUUID}"
    ).mock(
...
async def test_by_riot_id(pyke_client: Pyke, respx_mock: MockRouter):
    respx_mock.get(
        f"{BASE}/riot/account/v1/accounts/by-riot-id/saves/000"
    ).mock(
        return_value=Response(
            200, json={"puuid": PUUID, "gameName": "saves", "tagLine": "000"}
        )
...
async def test_region_by_puuid(pyke_client: Pyke, respx_mock: MockRouter):
    respx_mock.get(
        f"{BASE}/riot/account/v1/region/by-game/lol/by-puuid/{PUUID}"
    ).mock(return_value=Response(200, json={"puuid": PUUID, "region": "EUW"}))
